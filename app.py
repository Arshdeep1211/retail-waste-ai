...
def render_alerts(risk_df: pd.DataFrame) -> None:
    if risk_df.empty:
        st.markdown(
...
with left:
    ...
    st.markdown(
        '<div class="small-muted">Choose a store and optionally upload fresh datasets to simulate a live software workflow.</div>',
        unsafe_allow_html=True
    )
...
with tab_data:
    ...
    st.markdown(
        '<div class="small-muted">Upload your operational files to simulate a live retail software workflow.</div>',
        unsafe_allow_html=True
    )
...
with tab_dashboard:
    ...
    with c1:
        st.markdown(
            f'<div class="kpi"><div class="kpi-label">Tracked SKUs</div><div class="kpi-value">{total_skus}</div><div class="kpi-note">Active assortment monitored</div></div>',
            unsafe_allow_html=True
        )
    with c2:
        st.markdown(
            f'<div class="kpi"><div class="kpi-label">High Risk Items</div><div class="kpi-value">{high_risk}</div><div class="kpi-note">Immediate attention required</div></div>',
            unsafe_allow_html=True
        )
    with c3:
        st.markdown(
            f'<div class="kpi"><div class="kpi-label">Medium Risk Items</div><div class="kpi-value">{med_risk}</div><div class="kpi-note">Monitor and plan response</div></div>',
            unsafe_allow_html=True
        )
    with c4:
        st.markdown(
            f'<div class="kpi"><div class="kpi-label">Total Stock</div><div class="kpi-value">{total_stock}</div><div class="kpi-note">Units currently on hand</div></div>',
            unsafe_allow_html=True
        )
...
with tab_copilot:
    ...
    st.markdown(
        '<div class="small-muted">Ask operational questions in natural language and get inventory-aware guidance.</div>',
        unsafe_allow_html=True
    )
